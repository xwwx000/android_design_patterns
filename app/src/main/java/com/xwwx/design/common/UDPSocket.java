...
import com.xwwx.design.bean.Users;
...
public class UDPSocket {
    ...
    private void startSocketThread() {
        ...
        clientThread.start();
        Log.d(Contants.TAG, "开启 UDP 数据接收线程");
    ...
    private void receiveMessage() {
        while (isThreadRunning) {
            try {
                ...
                lastReceiveTime = System.currentTimeMillis();
                Log.d(Contants.TAG, "receive packet success...");
            } catch (IOException e) {
                Log.e(Contants.TAG, "UDP数据包接收失败！线程停止");
                stopUDPSocket();
                e.printStackTrace();
                return;
            }
            ...
            if (receivePacket == null || receivePacket.getLength() == 0) {
                Log.e(Contants.TAG, "无法接收UDP数据或者接收到的UDP数据为空");
                continue;
            }

            String strReceive = new String(receivePacket.getData(), receivePacket.getOffset(), receivePacket.getLength());
            Log.d(Contants.TAG, strReceive + " from " + receivePacket.getAddress().getHostAddress() + ":" + receivePacket.getPort());
    ...
    public void startHeartbeatTimer() {
        ...
        timer.setOnScheduleListener(new HeartbeatTimer.OnScheduleListener() {
            ...
            public void onSchedule() {
                Log.d(Contants.TAG, "timer is onSchedule...");
                long duration = System.currentTimeMillis() - lastReceiveTime;
                Log.d(Contants.TAG, "duration:" + duration);
                if (duration > TIME_OUT) {//若超过两分钟都没收到我的心跳包，则认为对方不在线。
                    Log.d(Contants.TAG, "超时，对方已经下线");
                    // 刷新时间，重新进入下一个心跳周期
                    lastReceiveTime = System.currentTimeMillis();
        ...
        mThreadPool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    InetAddress targetAddress = InetAddress.getByName(serverIp);
